...
#             print(i)
            if line != '\n':
                ...
                #     y_issue.append([0, 1])
                x_dialog, y_dialog, y_issue, y_multi = [], [], [], []
    return x, y, y_issue_ret, y_multi_ret

x_train, y_train, y_train_issue, y_train_multi = load_data_and_labels(train_file, 'train')
x_valid, y_valid, y_valid_issue, y_valid_multi = load_data_and_labels(valid_file, 'valid')
# x_test, y_test, y_test_issue = load_data_and_labels(test_file)
x_test, y_test, y_test_issue, y_test_multi = load_data_and_labels(test_file, 'test')
...
pred_val = model2.predict([x_val_single, x_val_feat_with_context_single, x_val_with_context_single])
